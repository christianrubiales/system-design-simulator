...
export const SERVICE_CONFIG: Record<string, ServiceConfigSpec> = {
  ...
  // --- Extended catalog: fixed capacity, no instance modelling yet ---
  "transit-gateway": fixed(1000000, "Scales to 50 Gbps per attachment; not a request-path bottleneck."),
  "direct-connect": fixed(1000000, "Dedicated circuits from 50 Mbps to 100 Gbps; latency depends on physical distance."),
  batch: fixed(1000, "Job-shaped, not request-shaped; throughput depends entirely on job duration."),
  "app-runner": fixed(5000, "Assumes a light API at roughly 5,000 req/s per service instance."),
  beanstalk: fixed(5000, "Provisions EC2 behind an ALB; per-instance throughput matches EC2."),
  lightsail: fixed(2000, "Small fixed-size instances; models a modest bundle."),
  "auto-scaling": fixed(1000000, "A control plane, not a request-path hop."),
  ecr: fixed(10000, "Registry pull throughput; off the request path in steady state."),
  backup: fixed(1000, "Scheduled jobs, not request traffic."),
  fsx: fixed(100000, "Lustre scales to hundreds of GB/s; figure models a mid-size deployment."),
  ses: fixed(14, "Default production send rate starts at 14 messages/second and grows with reputation."),
  mq: fixed(10000, "Broker-instance dependent; models a mid-size single broker."),
  "iot-core": fixed(20000, "Default account quotas are in the tens of thousands of messages/second."),
  emr: fixed(500, "Batch cluster throughput is job-shaped, not request-shaped."),
  shield: fixed(1000000, "Operates inline at the edge; adds no measurable request latency."),
  acm: fixed(1000000, "A control plane, not a request-path hop."),
  cloudtrail: fixed(100000, "Audit logging, off the request path."),
  "systems-manager": fixed(10000, "Control-plane operations; Parameter Store reads are cached by clients."),
  organizations: fixed(1000000, "Governance, not a request-path hop."),
  bedrock: fixed(1000, "Model-dependent; quotas are expressed in tokens per minute, not QPS."),
  sagemaker: fixed(5000, "Endpoint throughput depends on instance type and model size."),
  rekognition: fixed(50, "Default is roughly 50 transactions/second per operation, raisable."),
  textract: fixed(10, "Synchronous operations default to single-digit TPS; async for large documents."),
  mediaconvert: fixed(100, "Job-shaped; transcode duration scales with video length and rendition count."),
  codepipeline: fixed(100, "Pipeline executions, not request traffic."),
  codebuild: fixed(100, "Build-shaped; duration dominates and varies hugely by project."),
  codedeploy: fixed(100, "Deployment operations, not request traffic."),
  amplify: fixed(50000, "Served from a CDN; throughput follows CloudFront rather than an origin."),
};
