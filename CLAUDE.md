...
- **Time-stepped simulation (`engine/ticks.ts`).** 120 one-second ticks wrapping `runSimulation`. **The tick ordering is fixed and load-bearing:** arrivals (scenario + retries from t−1) → propagate/serve at the capacity the tick *started* with → queues absorb, others shed → record → autoscaling applies to t+1 → retries apply to t+1. Both feedback paths lag one tick; that lag is *why* autoscaling cannot rescue a spike, and it removes within-tick circularity. **A queue drains at its CONSUMER's capacity, not its own** — measuring overflow against the queue's own ceiling makes backlog permanently zero, which is the first bug this model had. Retries are hard-capped at 5× baseline and flagged as a storm past 2×. **The steady-state engine is the oracle:** `check-catalog` asserts a ticked steady run reproduces `runSimulation` within 1% across all 35 reference solutions, so a tick-model bug fails the build. Scoring always uses `steady` so grades stay comparable.
